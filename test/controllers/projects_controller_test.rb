...
class ProjectsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "update_llm sets the project's provider and model from the combined combobox value" do
    project = Project.create!(name: "Demo", repo_folder_path: @repo_dir)
    patch update_llm_project_url(project), params: { project: { llm: "ollama|qwen3.6:latest" } }
    assert_redirected_to projects_url
    assert_equal "ollama", project.reload.llm_provider
    assert_equal "qwen3.6:latest", project.llm_model
  end

  test "update_llm keeps model ids containing slashes intact" do
    project = Project.create!(name: "Demo", repo_folder_path: @repo_dir)
    patch update_llm_project_url(project), params: { project: { llm: "openrouter|moonshotai/kimi-k2.7-code" } }
    assert_redirected_to projects_url
    assert_equal "openrouter", project.reload.llm_provider
    assert_equal "moonshotai/kimi-k2.7-code", project.llm_model
  end
  ...
  test "update_llm clears the override back to the app default" do
    ...
    patch update_llm_project_url(project), params: { project: { llm: "" } }
    assert_redirected_to projects_url
